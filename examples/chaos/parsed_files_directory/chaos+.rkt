(module chaos racket
  ...
  (define-infix (g x y) (abs (sin (sqrt (x ** 2 + y ** 2)))))
  ...
  (define (scalar-to-long-rainbow-rgb s)
    ...
    ($nfx$ y := (inexact->exact (floor (255 * (a - x)))))
    (case x
      ((0) (values 255 y 0))
  ...
  (define (yellow-to-red s)
    ($nfx$ a := 1 - s)
    ($nfx$ y <- (inexact->exact (floor (255 * a))))
    (values 255 y 0))
  ...
  (define-infix
   ...
   ($nfx$ h <- (sqrt ((norm x y) / max-norm-x-y)))
   (when (h = 1.0) (h <- 0.999))
  ...
  (define (compute-rgb-color point max-norm-x-y)
    ...
    ($nfx$ y <- ($bracket-apply$ point 1))
    (define red-val
      (inexact->exact (abs (round ($nfx$ 255 * (red-value x y))))))
    (define green-val
      (inexact->exact (abs (round ($nfx$ 255 * (green-value x y))))))
    (define blue-val
      (inexact->exact (abs (round ($nfx$ 255 * (blue-value x y))))))
    ($nfx$ s <- (sqrt ((norm x y) / max-norm-x-y)))
    ...
    (when graphic-mode
      ($nfx$
       ...
       (new
        ...
        (width xws)
        (height yws)))
      (send ($nfx$ ($bracket-apply$ frm index)) show #t)
