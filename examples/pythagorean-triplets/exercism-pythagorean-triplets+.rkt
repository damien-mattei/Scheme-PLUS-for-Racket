...
(require ;(rename-in racket/contract (-> C>))
         racket/contract
	 ;; we rename the -> operator of Scheme+ (as it is rarely used, <- is the preferred and there exist alias := and =:)
	 ;; instead of renaming the -> operator of racket/contract
         (rename-in Scheme+ (-> =->))) 
...
(define (pythagorean-triplet? a b c)
  {a < b < c and a * a + b * b equal? c * c}) ; i suppose here we could have use = instead of equal? but as Scheme+ know equal? is a procedure but also an operator with precedence i let it be...
