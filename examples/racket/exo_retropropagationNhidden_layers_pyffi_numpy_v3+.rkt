...
;; > (require "exo_retropropagationNhidden_layers_pyffi_numpy_v3+.rkt")


(module exo_retropropagationNhidden_layers_pyffi_numpy_v3 racket
...
(display "exo_retropropagationNhidden_layers_pyffi_numpy_v3 : passed (require pyffi pyffi/numpy)") (newline)
...
(overload-procedure uniform uniform-dummy (number?)) ; DEPRECATED was used with flomat

(overload-procedure uniform uniform-interval (number? number?))

(define (uniform-doume) {-1 + (random) * 2})

(overload-procedure uniform uniform-doume ())
...
which works with Scheme+ parser
dim return values from numpy.shape for a given numpy.array
|#
(define (dim A)
  {shp <- (numpy.shape A)}
  {lin <- shp[0]}
...
(define ReseauRetroPropagation
  ...
  (class object%
	 ...
	 ;(display "after define-pointwise-unary") (newline)
         ...
         (define (createNumpyArray f n p)

           (numpy.array (list-ec (s42: i n) ; list by eager comprehension (SRFI 42)
                                 (list-ec (s42: j p)
                                          (f)))))
	 ...
	 (field (M (vector-ec (s42: n {lnc - 1}) ; vectors by eager comprehension (SRFI 42)
                              (createNumpyArray  uniform {nc[n + 1]} {nc[n] + 1})))) ;; numpy.array
					   
	 (display "M=") (display M) (newline)

	 ;(display (number? { M[1][0 0] })) (newline)
	 ;(display { M[1][0 0] }) (newline)
	 ...
	 (define (accepte_et_propage x) ; on entre des entrées et on les propage
		...
		(for+ ({i <- 0} {i < n - 2} {i <- i + 1}) ; personnal 'for' definition as in Javascript,C,C++,Java
		     ...
		     {z_1 <- #(1) + z[i]} ; + operator has been overloaded to append scheme vectors
   		     ...
   		     {z̃[i + 1] <- M[i] · z_1} ; z̃ = math matrix numpy array * vector , return a vector, operator overloaded in pyffi
	...
	(define/public (apprentissage Lexemples) ; apprentissage des poids par une liste d'exemples
	  ...
	  (for-racket ([it (range nbiter)]) ; le nombre d'itérations est fixé !
			...
			(display it)(newline))
		      ...
		      ;(display "apprentissage : passed accepte_et_propage") (newline)
			...
			{eror <- err})               ; mémorisation de l'erreur totale à la dernière itération
		      ...
		      ;(display (number? { M[i - 1][0 0] })) (newline)
		      ;(display { M[i - 1][0 0] }) (newline)
...
;{trunc_v <- (numpy.vectorize trunc)} ; vectorize not in pyffi

(define (apply2ndarray f A) ; apply f to ndarray elements of A
  {(lin col) <- (dim A)}
  (for+ ({l <- 0} {l < lin} {l <- l + 1})
    (for+ ({c <- 0} {c < col} {c <- c + 1})
      {A[l c] <- (f A[l c])})))
...
(for-racket ([mt M]) ; M is a Scheme Vector, not a "Matrix", mt is a numpy array
	    (apply2ndarray trunc mt)) 

;(for-racket ([i (range (vector-length (M)])    
;	(trunc-matrix mt)) 
